...
void main()
{
    ...
    gl_FragColor.rgb = mix(gl_FragColor.rgb, vec3(solid), anim.blendSolid);
    gl_FragColor.rgb = mix(gl_FragColor.rgb, lineColor, lineWidth * anim.blendLine);
    gl_FragColor.rgb = mix(gl_FragColor.rgb, meshColor, anim.blendMesh);
    
    // degamma
    gl_FragColor.rgb = pow(gl_FragColor.rgb, vec3(1.0 / 2.2));
    
    // displacement
    gl_FragColor.a = mix(0.0, normal.w * 4.0, anim.blendSolid);
